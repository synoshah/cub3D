...
void	render_start(t_context *ctx)
{
	...
	if (!ctx->title_img)
		ctx->title_img = mlx_xpm_file_to_image(ctx->mlx,
				"textures/hii.xpm", &w, &h);
	i = -1;
	while (++i < 800)
	{
		j = -1;
	...
	if (ctx->title_img)
		mlx_put_image_to_window(ctx->mlx, ctx->mlx_win,
			ctx->title_img, 350, 150);
	draw_start_text(ctx);
}

static void	animate_bamboo_wall(t_context *ctx)
{
	static int	frame = 0;
	t_data		temp;
