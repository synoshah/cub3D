...
static void	move_player(t_context *ctx, double move_x, double move_y)
{
	...
	pos_x = (int)ctx->player->pypos_x;
	pos_y = (int)ctx->player->pypos_y;
	if (new_x >= 0 && (size_t)new_x < ft_strlen(ctx->map->grid[pos_y]))
	{
		if (ctx->map->grid[pos_y][(int)new_x] != '1')
			ctx->player->pypos_x = new_x;
	}
	if (new_y >= 0 && (size_t)new_y < ctx->map->size.height
		&& (size_t)pos_x < ft_strlen(ctx->map->grid[(int)new_y]))
	{
		if (ctx->map->grid[(int)new_y][pos_x] != '1')
