...
int	reload_textures(t_context *ctx)
{
	...
	{
		if (!load_texture(ctx->mlx, &ctx->textures[4], "textures/throne.xpm"))
			return (0);
	}
	else if (ctx->game_state->level == 1)
	{
		if (!load_texture(ctx->mlx, &ctx->textures[4], "textures/boat.xpm"))
			return (0);
	}
	else
	{
	...
	{
		if (!load_texture(ctx->mlx, &ctx->textures[5], "textures/natural_green_bamboo.xpm"))
			return (0);
	}
	return (1);
}
...
int	load_next_level(t_context *ctx)
{
	...
	else if (ctx->game_state->level == 3 && ctx->keys->x)
		return (0);
	if (!ctx->map)
	{
